...
class MemoryBackend(Protocol):
    ...
    def update_item(
        self, item_id: str, user_id: str, collection_name: str, updates: Dict[str, Any]
    ) -> bool:
...
class QdrantMemoryBackend:
    ...
    def __init__(self, config: MemoryConfig):
        """Initialize Qdrant memory backend."""
        self._config = config
            ...
            search_result = self._client.scroll(
                ...
                scroll_filter=Filter(
                    must=[
                        FieldCondition(key="user_id", match=MatchValue(value=user_id)),
                        FieldCondition(
                            key="memory_item_id", match=MatchValue(value=item_id)
                        ),
                    ]
                ),
                limit=1,
...
class MemoryExtractor:
    ...
    def extract_info(
        ...
        available_types = extraction_schema.get(
            "types", ["personal", "preference", "project"]
        )
...
class MemoryManager:
    ...
    def search(
        ...
        return self._backend.search_items(
            user_id, query, collection_name, item_type, limit
        )
    ...
    def get_all(
        ...
        return self._backend.get_items(
            user_id, collection_name, item_type, filters, limit
        )
    ...
    def extract_and_store(
        ...
        extracted_items = self._extractor.extract_info(
            text, user_id, extraction_schema, context
        )
        ...
        for item in extracted_items:
            ...
            if dedup_limit > 0:
                existing_items = self.search(
                    user_id, item.content, collection_name, limit=dedup_limit
                )

                # Simple similarity check
                is_duplicate = any(
                    existing.content.lower().strip() == item.content.lower().strip()
                    or (
                        len(existing.content) > 10
                        and existing.content.lower() in item.content.lower()
                    )
                    or (
                        len(item.content) > 10
                        and item.content.lower() in existing.content.lower()
                    )
    ...
    def get_context_summary(
        self, user_id: str, collection_name: str, limit: int = 20
    ) -> str:
