...
Always use proper tool calling format and format the tool results nicely for the user."""
    ...
    try:
        ...
        if hasattr(response, 'content') and response.content:
            ...
            print(f"DEBUG: Starts with {{\"name\": {content.strip().startswith('{\"name\"')}")
            ...
            if ('{' in content and '}' in content and '"name"' in content and 
                ('list_todos' in content or 'todo_manager' in content or 'complete_todo' in content)):
                ...
                try:
                    ...
                    if start >= 0 and end > start:
                        ...
                        print(f"DEBUG: AGGRESSIVE - Tool: {tool_name}, Cleaned params: {cleaned_params}")
            ...
            if ("call list_todos" in content_lower or "to view all todos" in content_lower):
            ...
            if match:
                try:
                    ...
                    print(f"Failed to parse python_tag tool call (regex): {match.group(1)}, error: {e}")
                ...
                if json_part.startswith('{'):
                    ...
                    if end_idx > 0:
                        ...
                        try:
                            ...
                            print(f"DEBUG: Parsed python_tag tool call (manual): {tool_data}")
                        except json.JSONDecodeError as e:
                            print(f"Failed to parse manually extracted python_tag JSON: {complete_json}, error: {e}")
                            tool_data = None
                ...
                try:
                    ...
                    try:
                        ...
                        ultra_cleaned = content.strip().replace("\\'", "'").replace('\\"', '"')
                        print(f"DEBUG: Ultra-cleaned content: {ultra_cleaned}")
                        tool_data = json.loads(ultra_cleaned)
                        print(f"DEBUG: Parsed ultra-cleaned JSON tool call: {tool_data}")
                    except json.JSONDecodeError as e2:
                        print(f"Failed to parse ultra-cleaned JSON: {ultra_cleaned}, error: {e2}")
                        tool_data = None
            else:
                # Fallback: try to find any JSON-like structure in the content  
                print(f"DEBUG: No specific pattern matched, trying fallback JSON parsing")
                ...
                if json_match:
                    try:
                        ...
                        if end_pos > 0:
                            ...
                            print(f"DEBUG: Parsed fallback JSON (no brace counting): {tool_data}")
                    except json.JSONDecodeError as e:
                        print(f"Failed to parse fallback JSON: {potential_json}, error: {e}")
                        tool_data = None
                        
                # Last resort: try parsing the entire content if it looks like JSON
                elif '{' in content and '}' in content and '"name"' in content:
                    try:
                        print(f"DEBUG: Last resort - trying to parse entire content as JSON")
            ...
            if tool_data:
                ...
                print(f"DEBUG: Creating tool call - name: {tool_name}, original params: {tool_params}")
                ...
                print(f"DEBUG: Created AI response with tool calls: {ai_response.tool_calls}")
        ...
        has_tool_calls = (isinstance(response, AIMessage) and 
...
def should_continue(state: TodoAgentState) -> str:
    ...
    if len(messages) >= 3:
        ...
        if ("tool" in recent_msg_types and 
    ...
    if has_recent_tool_messages and is_ai_without_tools:
        print("DEBUG: Found recent tool execution and AI response without tool calls, ending")
        return "__end__"
