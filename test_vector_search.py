...
def search_qdrant(
    query_vector: List[float], limit: int = 10, score_threshold: float = 0.5
) -> Dict[str, Any]:
    ...
    print(
        f"Searching Qdrant collection '{COLLECTION_NAME}' with {len(query_vector)}D vector..."
    )
    response = requests.post(url, json=payload)

    if response.status_code != 200:
        raise Exception(
            f"Qdrant search failed: {response.status_code} - {response.text}"
        )
...
def check_qdrant_collection() -> Dict[str, Any]:
    ...
    if response.status_code == 404:
        ...
        raise Exception(
            f"Failed to check collection: {response.status_code} - {response.text}"
        )
...
def test_vector_search():
    ...
    try:
        ...
        if collection_info:
            ...
            print(
                f"   Vector size: {config.get('params', {}).get('vectors', {}).get('size', 'unknown')}"
            )
        else:
        ...
        if (
            collection_info
            and collection_info.get("result", {}).get("points_count", 0) > 0
        ):
            print(f"\n3. Searching Qdrant...")
            try:
